...
/*--------------------------------------------------------------------*/
#include <ctype.h>//REMOVED UNNECESSARY FILES
/*--------------------------------------------------------------------*/
...
static char nextchar(void)
{
  ...
  /*--------------------------------------------------------------------*/
  char *buf_in = "2 18 + = 5 / = ";
  static int index = 0;/*starts at 0 */ //ASSIGNED VALUE AS 0
  char ret;
  ret = buf_in[index];
  ++index;
  return ret;
...
Token read_token(char buf[])
{
  ...
  for (c = nextchar(); !isspace(c); c = nextchar()) //CORRECTED LOOP CONDITON
  {
